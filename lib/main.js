...
function(){
	...
	function onSuccess(array) {
		...
		if(oldSave){
			...
			var full_url = tabs.activeTab.url;
			var trimmed = full_url.substring(0,full_url.length - 1);
			console.log("trim: " + trimmed);
			tabUserp = oldSave[trimmed];
		}
...
function(saveObj){
	...
	function onFailure(){
		...
		saveContainer[tabs.activeTab.url] = saveObj;
